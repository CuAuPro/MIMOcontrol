...
poles_oz = eig(A);
nr_poles = length(poles_oz);

%desired_poles = [-1,-1,-2];
%q = [5;1;0];

desired_poles = [-0.04, -0.02, -0.002];
q = [1;1];
%syms s
%poly_oz = det(s*eye(size(A))-A);
%coeff_oz = fliplr(coeffs(poly_oz));
f = poly(poles_oz);
f = f(2:end);
h = poly(desired_poles);
h = h(2:end);

M = zeros(nr_poles,size(B',2));
...
P = q*p;
syms s
poly_zz = det(s*eye(size(A))-A+B*P);
coeff_zz = fliplr(coeffs(poly_zz));
novi_poli = roots(coeff_zz);
%model_zz = tf(ss(Azz,Bzz,Czz,Dzz));
fprintf("Stari poli: [%s]\n", join(string(poles_oz), ','))
fprintf("Zeljeni poli: [%s]\n", join(string(desired_poles), ','))
fprintf("Novi poli: [%s]\n", join(string(novi_poli), ','))
