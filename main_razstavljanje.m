...
r1_in.Times = [0, 2500, 7000, 10000];
% r2_in = {};
% r2_in.Vals = [r2_ss, r2_ss+d_r2, r2_ss-d_r2, r2_ss];
% r2_in.Times = [0, 4000, 6000, 12000];
r2_in = {};
r2_in.Vals = [r2_ss];
r2_in.Times = [0];
...
l = size(C,1);
if m == l
    fprintf("Sistem je kvadraten.\n");
else
    fprintf("Sistem ni kvadraten. To ni OK!\n");
end

if sum(D,'all') == 0
    fprintf("Sistem je striktno pravi.\n");
else
    fprintf("Sistem ni striktno pravi. To ni OK!\n");
end
...
for i=1:m
    ...
    cond_di = 0;
    for j=0:nr_poles-1-1
        if sum(C(j+1,:)*A^j*B, 'all') ~= 0
            d_i(j+1) = j;
            cond_di = 1;
            break
        end
...
Bstar = zeros(l,m);
for i=1:m
    Bstar(i,:) = C(i,:)*A^(d_i(i))*B;
end

if det(Bstar) ~= 0
    fprintf("Sistem je funkcionalno vodljiv in razstavljiv.\n");
else
    fprintf("Sistem ni funkcionalno vodljiv in razstavljiv. To ni OK!\n");
end

Astar = zeros(l,nr_poles);
for i=1:m
    Astar(i,:) = C(i,:)*A^(d_i(i)+1);
end
...
%desired_poles = [-1,-2,-3];
desired_poles = [-0.08, -0.04];
delta = max(d_i);
Mk = diag(desired_poles);
%Mk je nacelomaena podmatrika. Teh matrik je (delta)
...
model_zz = tf(ss(Azz,Bzz,Czz,Dzz));
fprintf("Stari poli: [%s]\n", join(string(poles), ','))
fprintf("Zeljeni poli: [%s]\n", join(string(desired_poles), ','))
fprintf("Novi poli: [%s]\n", join(string(eig(Azz)), ','))
...
G_REG = model_zz; %just copy structure...
...
%                 s           (s+1/tau)
%
% Ce je nicla integrirnega clena na mestu, kjer ima sistem pol (Ti = tau):
%
%            Ks*Kp                 
%           -------            
%            s*tau              1
...
%              s*tau           Ks*Kp
%
% S tem je vpliv PI regulatorja na zaprtozancni sistem zanemarljivo majhen.
% Ce pa zelimo vplivati na dinamiko pa lahko ojacenje regulatorja (Kp)
% nastavimo kot: Kp = tau/tau_zz*Ks
%
Kp = 1;
G_REG(1,1) = Kp*(s-pole(G_REG(1,1)))/s;
Kp = 1;
G_REG(2,2) = Kp*(s-pole(G_REG(2,2)))/s;
%%

%% Simulate


out = sim("razstavljanje.slx");
